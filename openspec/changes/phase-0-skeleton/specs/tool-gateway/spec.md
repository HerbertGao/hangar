...
批量 approve 中途某动作重试耗尽失败 → 经 choke-point 该 run 收束 `run.failed`(已 `granted` 的不回滚、记录已执行;其余 pending 置 `superseded`)。
...
### 需求:reject 驳回不执行
对 `waiting_human` 的 run(已取锁),`reject` 不执行任何动作:pending `Approval` 置 `rejected`,经 choke-point 收束 `run.cancelled`。
...
Phase 0 假设**单用户单进程**审批。上面的 run-state 守卫 + approve 取锁 + reaper + 幂等键已覆盖「崩溃恢复」与「第二个 approver 被守卫挡掉」。**多进程同时 approve/reject 同一 run 的完整仲裁**(seq 竞争败者重试、approve-vs-reject 活 race、granting 的 lease/超时回收)**显式延后 Phase 1**(inbox 每天 cron + 可能多入口时才真需要)。这是有意的、已披露的范围延后,不是遗漏。
