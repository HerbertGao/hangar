...
5. 部署:plist `EnvironmentVariables` 加 `TG_BOT_INBOX` + `HANGAR_NOTIFY_CONFIG`(或用约定默认路径),保证 `hangar run` 入口也拿得到;放 `channels.yaml`;**在 daemon 的 env 里跑 `hangar-notify check`**;切换。
