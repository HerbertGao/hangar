...
- **更新 DESIGN.md**(§3.4 reaper 边界 + 能力闸;§6 仅补注 (d) 不引入进程外 worker--**不改 §6 假设**)--改架构先改 DESIGN(#9)。
...
- **契约文档**:`DESIGN.md`(§3.4 reaper 边界;§6 **假设不改、仅补一条注**--(d) 留在进程内)、`SKILL.md`(doctor 能力集若含 `hardcrash` 闸,按平台标 best-effort)。
- **数据/DB + 进程(#3)**:handle = **`run_id`**(createRun 起就 durable)。**两平台均零新列**(Linux cgroup 路径由 run_id 派生、macOS tag 即 run_id)。**(d) pipeline 在进程内、不加常驻第二进程**(ctx.spawn 的子进程 = pipeline 今天就在起的 build/git,只是走受闸入口)--故 **§6 假设不改(仅补一条注)、`#6` 不碰**,守 #3(不加表、不加列、不加进程)--比 worker 方案轻。
...
- **不承诺在 macOS 上「可证明」回收**:macOS 无内核容器,担保残余 = **任何丢掉 env-tag** 的子孙(密封沙箱构建 scrub env)+ env 不可读的 uid-change/SIP 子进程 + inspect-to-kill PID 复用误杀窗口;可证明须 Linux(有 cgroup 委派)或 Linux VM。ADR 如实标注这道平台天花板。
