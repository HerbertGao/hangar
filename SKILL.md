...
**约定:** 所有命令 `--json` 输出结构化结果到 stdout;日志到 stderr;退出码 `0` 成功 / `1` 业务失败 / `2` 参数错误。写操作拒绝 root。
...
```
**错误 kind:** `sqlite_unwritable` · `apps_dir_missing` · `spec_invalid` · `pipeline_missing`。
**Agent 约定:** 任一 check 非 `ok` 时,先修环境,别急着 `run`。

## `hangar status [--json]`

**用途:** 所有 app 当前状态一览(读 SQLite,只读)。
**返回:** `[{ "app": "inbox", "lastRun": "run_a3f", "state": "waiting_human", "since": "<iso>" }]`
...
**返回:** `{ "run": "run_a3f", "state": "waiting_human|completed|failed" }`
**错误 kind:** `app_not_found` · `spec_invalid` · `already_running`(run 锁)。

## `hangar approve <run> [--json]`

**用途:** 执行该 run 的全部待批动作,续跑至 `completed`。**写操作,拒绝 root。**
**返回:** `{ "run": "run_a3f", "state": "completed", "executed": [ { "tool": "gmail.send", "ok": true } ] }`
**错误 kind:** `run_not_found` · `no_pending_approval` · `action_failed`(动作执行失败,含 redacted error)。
**Agent 约定:** 高危动作(发送/写入),approve 前**务必向人确认**动作内容,不代人拍板。
