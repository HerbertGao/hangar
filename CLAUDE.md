...
4. **app 定义唯一入口 `app.yaml`,方向由 `executor` 定。** `pipeline` → 约定加载同目录 `pipeline.ts`。不引入第二种 app 定义方式(不加 `defineApp`、不加 JSON manifest、不加字符串入口)。
...
- 目录:`packages/core`(脊柱)、`apps/<id>/`(pilot)、`hangar.sqlite`(状态)。
